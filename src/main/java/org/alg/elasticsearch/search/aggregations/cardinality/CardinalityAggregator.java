 ...
 *
 */
public class CardinalityAggregator extends SingleBucketAggregator {
    
    private ValuesSource valuesSource;
    
    private ObjectArray<HyperLogLogPlus> counters;

    public CardinalityAggregator(String name, AggregatorFactories factories, long estimatedBucketsCount, ValuesSource valuesSource, AggregationContext aggregationContext, Aggregator parent) {
    ...
    public InternalAggregation buildAggregation(long owningBucketOrdinal) {
        return new InternalCardinality(name, counters == null || owningBucketOrdinal >= counters.size() ? null : counters.get(owningBucketOrdinal));
    }
    ...
    public static class Factory extends ValueSourceAggregatorFactory<ValuesSource> {
        public Factory(String name, ValuesSourceConfig<ValuesSource> valueSourceConfig) {
            super(name, InternalCardinality.TYPE.name(), valueSourceConfig);
        }
        ...
        public Aggregator create(ValuesSource valuesSource, long expectedBucketsCount, AggregationContext aggregationContext, Aggregator parent) {
            return new CardinalityAggregator(name, factories, expectedBucketsCount, valuesSource, aggregationContext, parent);
        }

        @Override
        protected Aggregator createUnmapped(AggregationContext aggregationContext, Aggregator parent) {
            return new CardinalityAggregator(name, factories, 0, null, aggregationContext, parent);
        }
    }
